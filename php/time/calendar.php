...
/*     $firstDay=date("Y-m-01");
    $month=date("m"); */
    $firstDay=date("Y-m-01"); //設變數為目前年度月份的01日
    $month=date("m"); //設變數為當前月份
    $firstWeekWhiteDays=date("w",strtotime($firstDay)); //判斷$firstDay是星期幾 即可知道第一天之前有幾個空白天
    $monthDays=date("t",strtotime($firstDay)); //判斷$firstDay這個月有幾天
    $firstWeekDays=7-$firstWeekWhiteDays; //第一周有幾個實際天數
    $weeks=ceil(($firstWeekWhiteDays+$monthDays)/7); //這個月需列出幾周 無條件進位計算[(第一周空白天+這個月總天數)/7]
    $lastWeekDays=($firstWeekWhiteDays+$monthDays)%7; //設變數為最後一周有幾個實際天數
    $lastWeekWhiteDays=7-$lastWeekDays; //設變數為最後一周需填入幾個空白天
    $allCells=($weeks+1)*7; //
    //陣列中加入首列資料
